...
FujicoinApplication::FujicoinApplication(interfaces::Node& node, int &argc, char **argv):
...
static void SetupUIArgs()
{
    ...
    gArgs.AddArg("-uiplatform", strprintf("Select platform to customize UI for (one of windows, macosx, other; default: %s)", FujicoinGUI::DEFAULT_UIPLATFORM), true, OptionsCategory::GUI);
}
...
int GuiMain(int argc, char* argv[])
{
    ...
    FujicoinApplication app(*node, argc, argv);
    ...
    /// 6. Determine availability of data and blocks directory and parse fujicoin.conf
