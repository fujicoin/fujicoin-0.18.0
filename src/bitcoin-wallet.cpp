...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        std::string usage = strprintf("%s fujicoin-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                                      "wallet-tool is an offline tool for creating and interacting with Fujicoin Core wallet files.\n" +
...
int main(int argc, char* argv[])
{
    ...
    if (method.empty()) {
        fprintf(stderr, "No method provided. Run `fujicoin-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }
